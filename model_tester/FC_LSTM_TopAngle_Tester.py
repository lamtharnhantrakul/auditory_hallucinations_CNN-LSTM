...
audio_prefix = 'seq3a'
if USE_TITANX:
    data_dir = '/home/zanoi/ZANOI/auditory_hallucinations_data/FC_2_data/'
else:
    data_dir = '/Volumes/SAMSUNG_SSD_256GB/ADV_CV/data/FC_2_data/'
...
(num_frames, timesteps, features) = dataX.shape  # (1,8377,1,4096)
...
def loadModel():
    ...
    #model_idx = 2  # I'm just testing one model right now
...
for i in tqdm(range(num_windows)):
    ...
    plt.title('CNN-LSTM prediction using VGG-FC2 / Overfit on Top Angles only')
    ...
    plt.tight_layout()
    plt.draw()
    plt.savefig('../model_tester/FC_LSTM_TopAngle_figures/CNN_LSTM-pred_TopAngle' + str(i) + '.png')
    #plt.show()
    plt.close()
