...
## This code tests the model trained from scratch.
...
dataX = np.reshape(dataX_test,(-1, frame_h, frame_w, channels))
audio_vector_dim = dataY_test.shape[1]
dataY = np.reshape(dataY_test,(-1, audio_vector_dim))  # (8377,18)
...
for i in tqdm(range(num_windows)):
    ...
    plt.tight_layout()
    plt.draw()
    plt.savefig('../model_tester/CNN_LSTM_scratch_figures/CNN_LSTM_scratch-pred_TopAngle' + str(i) + '.png')
    #plt.show()
    plt.close()
